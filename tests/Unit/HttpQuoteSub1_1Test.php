...
class HttpQuoteSub1_1Test extends TestCase
{
    ...
    public function testCreateteSub1_1() {
        ...
        $response2 = $this->withSession(['member' => $member2])
            ->post("/quote/create/sub1-1/19", $paramEdit2);
        $response2->assertStatus(200)
            ->assertJson(function (AssertableJson $json) {
                $json->where('status', false)
                    ->where('msg', '輸入錯誤')
        ...
        $response3 = $this->withSession(['member' => $member2])
            ->post("/quote/create/sub1-1/19", $paramEdit3);
        $response3->assertStatus(200)
            ->assertJson(function (AssertableJson $json) {
                $json->where('status', false)
                    ->where('msg', '子資料已存在');
        ...
        $response4 = $this->withSession(['member' => $member2])
            ->post("/quote/create/sub1-1/20", $paramEdit3);
        $response4->assertStatus(200)
            ->assertJson(function (AssertableJson $json) {
                $json->where('status', true)
                    ->where('msg', 'success');
            });
        $sub1_1at1 = $quoteRepo->getSub1_1ByMainId(20);
        $this->assertEquals('SD00019', $sub1_1at1->partNo);
        $this->assertEquals('大東山於另', $sub1_1at1->materialName);
        $this->assertEquals(400, $sub1_1at1->length);
        $this->assertEquals('5夾', $sub1_1at1->specIllustrate);
        $this->assertEquals('A/A', $sub1_1at1->level);
        $this->assertEquals('N', $sub1_1at1->fsc);
        $this->assertEquals(600, $sub1_1at1->materialPrice);
    }
}
